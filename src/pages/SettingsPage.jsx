...
import { Settings, Film, Tv, Music, Languages, Activity, Bell, Library, Search, CheckCircle, XCircle, Loader2, Eye, EyeOff } from 'lucide-react';
      ...
      {config.enabled && (
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              placeholder={`http://localhost:${service.key === 'radarr' ? '7878' : service.key === 'sonarr' ? '8989' : service.key === 'lidarr' ? '8686' : service.key === 'bazarr' ? '6767' : service.key === 'tautulli' ? '8181' : service.key === 'overseerr' ? '5055' : service.key === 'plex' ? '32400' : '9696'}`}
              value={config.url}
      ...
      <div className="space-y-4 max-w-2xl">
        <div className="bg-muted/30 rounded-xl p-4 mb-6 border border-border/50">
          <p className="text-sm text-muted-foreground">
            <strong className="text-foreground">Important:</strong> MediaHub now proxies service traffic through its backend, so your browser no longer needs direct CORS access to Radarr, Sonarr, Bazarr, Tautulli, Overseerr, Plex, or Prowlarr.
            API keys and service settings are stored in MediaHub SQLite data instead of browser local storage.
          </p>
        </div>
